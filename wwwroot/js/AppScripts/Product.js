...
window.onload = function breadcrumbs() {
    document.getElementById('titleheader').innerHTML = 'Product';
    document.getElementById('breadLevel1').innerHTML = 'Catalog';
    document.getElementById('breadLevel2').innerHTML = 'Product';
    document.getElementById("breadLevel2").href = 'Product';
    document.getElementById('staticBackdropLabel').innerHTML = 'Product';
...
$(document).ready(function () {
    dataTable = $("#prodTable").DataTable({
        ...
        "columns": [
            ...
            { "data": "ProductGroup", "className": "text-left" },
            { "data": "ProductModel", "className": "text-left" },
            { "data": "ProductColor", "className": "text-left" },
            {
...
$('#prodTable').on('click', 'tbody .del_btn', function () {
    var data = dataTable.row($(this).closest('tr')).data();
    g_prodid = data.Id;
    document.getElementById('msgtext').innerHTML = 'Are you sure to delete this Product - ' + data.ProductName + ' ?';
});

$('#prodTable').on('click', 'tbody .edit_btn', function () {
    var data = dataTable.row($(this).closest('tr')).data();
    g_prodid = data.Id;
});
...
showInPopup = (id) => {
    $.ajax({
        ...
        success: function (res) {
            ...
            if (id == 0) {
                $('#form-modal .modal-title').html('Product - Create');
                fillSelectBoxes(0, 0, 0, 0);
            } else {
                $('#form-modal .modal-title').html('Product - Edit');               
                $.ajax({
                    type: 'GET',
                    async: false,
                    ...
                    success: function (result) {
                        ...
                        $("#PrdColList").val(result.data[0].ProdGrpId);

                        console.log('from getbyid : ' + $("#PrdColList").val());

                        //alert('prodjs : ' + result.data[0].ProdGrpId + " " + result.data[0].ProdModId + " " + result.data[0].ProdColId);
                        fillSelectBoxes(id, result.data[0].ProdGrpId, result.data[0].ProdModId, result.data[0].ProdColId);

                        //alert(document.getElementById('PrdColList').val());
